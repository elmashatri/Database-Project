...
Select H.lexuesi 
From huazimi H
Where (H.data_e_leshimit>='2020-01-01' or H.data_e_leshimit='2021-12-31' AND H.verejtje='Me vonesë');
...
//*trigger nr1**/
delimiter $$
create trigger CheckPaga before insert on punëtori
for each row
begin
IF NEW.Paga > 450
 THEN signal sqlstate '45000'  
 set message_text = 'Rroga nuk mundë të jetë me shumë se 450 euro';
END IF;
end $$
delimiter $$;
...
/**trigger  nr2**/
delimiter $$
create trigger data_e_leshimit before insert on lexuesit 
for each row 
